...
3. Убедиться, что в загрузках локально у вас есть тестовые данные для прогона: `train` и/или `test` файлики
4. Указать в терминале `docker-compose up`
5. Скопировать URL в браузер для поднятия Streamlit: `http://localhost:8501`
6.  Далее следовать инструкциям в Streamlit

# Ретро Readme для HW1
...
4. Запустить сервис:
   ...
   ```
5. Запустить в соседнем окне терминала docker для minio:
   ```
   docker run -p 9000:9000 minio/minio server /data
   ```
6. Запустить в соседнем окне терминала стримлит:
   ```
   streamlit run mlops_pipeline/streamlit.py
   ```
7. Убедиться, что в загрузках локально у вас есть тестовые данные для прогона: `train` и/или `test` файлики
8. Далее следовать инструкциям в Стримлите
...
1. Добавили работу с S3 (Minio), для сохранения туда артефактов - моделей, обученных пользователем через взаимодействие с streamlit, и датасетов, на которых происходило обучение.
3. Датасеты сохраняются и локально (папка `datasets`), и в minio в формате dvc. 
