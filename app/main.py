...
async def train_model(request: TrainRequestModel):
    ...
    task_url = f"https://app.clear.ml/projects/{project}/experiments/{task.id}/output/log"
...
async def predict(request: PredictRequestModel) -> Dict[str, Any]:
    ...
    try:
        ...
        task_logger.report_text(f"Data split successfully: x_test shape: {x_test.shape}, y_test shape: {y_test.shape}")
        ...
        task_logger.report_text(f"Prediction completed: accuracy = {accuracy:.4f}, ROC AUC = {roc_auc:.4f}")
