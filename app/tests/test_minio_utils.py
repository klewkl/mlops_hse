...
def test_upload_model_to_minio_success(mock_minio_client):
    ...
    mock_minio_client.fput_object.assert_called_once_with(
        bucket_name, object_name, local_model_path
    )
...
def test_upload_dataset_to_minio_and_track_with_dvc(mock_minio_client):
    ...
    with mock.patch("subprocess.run") as mock_run:
        ...
        upload_dataset_to_minio_and_track_with_dvc(
            local_dataset_path, bucket_name, object_name
        )

        mock_minio_client.fput_object.assert_called_once_with(
            bucket_name, object_name, local_dataset_path
        )
        mock_run.assert_any_call(["dvc", "add", local_dataset_path])
        mock_run.assert_any_call(
            ["git", "commit", "-m", f"Track dataset: {object_name}"]
        )
...
def test_upload_dataset_to_minio_and_track_with_dvc_failure(mock_minio_client):
    ...
    with mock.patch("subprocess.run") as mock_run:
        ...
        assert (
            result
            == "Error in DVC operations: Command 'dvc' returned non-zero exit status 1."
        )
