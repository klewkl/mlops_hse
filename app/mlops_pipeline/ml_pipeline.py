    ...
    if split:
        if "split" in data.columns:
            ...
            train, test = train_test_split(
                data_numeric, test_size=test_size, random_state=random_state
            )
...
class Model:
    ...
    def filter_params(self, model_params, model_type):
        params_dict = model_params.dict(
            exclude_unset=True
        )  # Convert model params to dictionary

        if model_type == ModelClass.logreg:
            return {
                key: value
                for key, value in params_dict.items()
                if key in ["penalty", "C", "max_iter"]
            }
            ...
            return {
                key: value
                for key, value in params_dict.items()
                if key in ["n_neighbors", "algorithm", "weight"]
            }
            ...
            return {
                key: value
                for key, value in params_dict.items()
                if key in ["max_depth", "min_samples_split", "min_samples_leaf"]
            }
    ...
    def save(self, model_dir: str):
        ...
        if self.model is None:
            raise ValueError(
                "Model is not initialized. Cannot save a model that is None."
            )
    ...
    def load(self, model_dir: Path, model_type: Optional[str] = None):
        if self.model is None:
            if model_type is None:
                raise ValueError(
                    "Model class has not been initialized and no model type provided."
                )
            ...
            try:
                ...
                raise ValueError(
                    f"Invalid model type: {model_type}. Must be one of {', '.join(ModelClass.__members__.keys())}"
                )
